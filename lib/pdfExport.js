...
export const exportToPDF = async (element, filename = 'cv.pdf', options = {}) => {
  const {
    scale = 3, // Increased from 1.5 to 3 for better resolution
    useCORS = true,
    allowTaint = true,
    backgroundColor = '#ffffff',
    width = 210, // A4 width in mm
    height = 297, // A4 height in mm
    quality = 1.0, // Maximum quality (1.0 for PNG, 0.98 for JPEG)
    imageFormat = 'PNG' // PNG for sharper text, JPEG for smaller file size
  } = options;
  ...
  try {
    ...
    // Convert canvas to image (PNG for better quality, JPEG for smaller size)
    const imgData = canvas.toDataURL(`image/${imageFormat.toLowerCase()}`, quality);
    
    // Add image to PDF (simplified approach)
    if (imgHeight <= maxHeight) {
      // Fits on one page
      pdf.addImage(imgData, imageFormat, margin, margin, imgWidth, imgHeight);
    } else {
      ...
      while (position < imgHeight) {
        ...
        const pageImgData = tempCanvas.toDataURL(`image/${imageFormat.toLowerCase()}`, quality);
        pdf.addImage(pageImgData, imageFormat, margin, margin, imgWidth, pageImgHeight);
